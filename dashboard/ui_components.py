...
def render_verdict_card(verdict_data):
    """
    Renders a sophisticated final verdict card.
    Expects verdict_data with: score, label, icon, color, verdict_text, pillar_scores
    """
    import streamlit as st
    
    score = verdict_data.get('score', 0)
    label = verdict_data.get('label', 'N/A')
    icon = verdict_data.get('icon', 'ℹ️')
    color = verdict_data.get('color', 'grey')
    text = verdict_data.get('verdict_text', '')
    pillars = verdict_data.get('pillar_scores', {})
    ...
    if pillars:
        ...
        for k, v in pillars.items():
            pillar_items.append(
                f'<div style="background: rgba(255,255,255,0.08); padding: 5px 12px; border-radius: 20px; font-size: 0.85rem; color: #fff;">'
                f'<span style="color: #8b949e;">{k}:</span> {v}</div>'
            )
        pillar_html = f'<div style="display: flex; gap: 15px; flex-wrap: wrap; margin-top: 20px;">{" ".join(pillar_items)}</div>'

    # Custom CSS for the verdict card (Glassmorphism + Neon)
    html_content = f"""
    <div style="background: rgba(255, 255, 255, 0.05); backdrop-filter: blur(10px); border-radius: 15px; padding: 24px; border-left: 8px solid {color}; margin: 20px 0; box-shadow: 0 4px 15px rgba(0,0,0,0.3);">
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div style="display: flex; align-items: center;">
                <span style="font-size: 2.5rem; margin-right: 15px;">{icon}</span>
                <div>
                    <h2 style="margin: 0; color: {color}; font-size: 1.8rem;">{label}</h2>
                </div>
            </div>
            <div style="text-align: right;">
                <div style="font-size: 3rem; font-weight: 800; color: #fff; line-height: 1;">{score:.0f}<span style="font-size: 1rem; color: #8b949e;">/100</span></div>
            </div>
        </div>
    </div>
    """
