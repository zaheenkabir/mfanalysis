...
def render_portfolio_view(nav_data, selected_funds):
    ...
    st.header("🎯 Build & Analyze Portfolio [v2-CLEAN]")
        ...
        if selected_scenario:
            ...
            if benchmark_data:
                ...
                if portfolio_result['data_available']:
                    ...
                    with col_s1:
                        metric_card("Portfolio Return", f"{portfolio_result['portfolio_return']:.2f}%", delta=portfolio_result['portfolio_return'], is_good_if_positive=True)
                    with col_s2:
                        metric_card("Portfolio Max DD", f"{portfolio_result['portfolio_max_drawdown']:.2f}%", delta=portfolio_result['portfolio_max_drawdown'], is_good_if_positive=True)
                    with col_s3:
                        rec_days = portfolio_result['days_to_recover'] or "Pending"
                        metric_card("Days to Recover", rec_days, None, "#AAA")
                    
                    # Benchmark Metrics Row
                    st.markdown("#### 📈 Benchmark Performance")
                    col_b1, col_b2, col_b3 = st.columns(3)
                    with col_b1:
                        metric_card("Benchmark Return", f"{portfolio_result['benchmark_return']:.2f}%", delta=portfolio_result['benchmark_return'], is_good_if_positive=True)
                    with col_b2:
                        metric_card("Benchmark Max DD", f"{portfolio_result['benchmark_max_drawdown']:.2f}%", delta=portfolio_result['benchmark_max_drawdown'], is_good_if_positive=True)
                    with col_b3:
                    ...
                    comparison_data = {
                        'Metric': ['Return During Crisis', 'Maximum Drawdown', 'Recovery Time'],
                        'Your Portfolio': [
                            f"{portfolio_result['portfolio_return']:.2f}%",
                            f"{portfolio_result['portfolio_max_drawdown']:.2f}%",
                            f"{portfolio_result['days_to_recover']} days" if portfolio_result['days_to_recover'] else "Not yet recovered"
                        ],
                        'Benchmark Index': [
                        ...
                        'Difference': [
                            f"{portfolio_result['portfolio_return'] - portfolio_result['benchmark_return']:+.2f}%",
                            f"{portfolio_result['portfolio_max_drawdown'] - portfolio_result['benchmark_max_drawdown']:+.2f}%",
                            "-"
                        ]
                    }
