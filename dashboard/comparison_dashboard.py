...
def render_comparison_dashboard(nav_data, selected_funds, selected_benchmarks=["NIFTY 50"]):
    ...
    st.markdown("### 🛡️ Crash Resilience (Stress Testing)")
    st.caption("Historical performance during major market crashes")
    
    scenarios = get_predefined_scenarios()
    selected_scenario_name = st.selectbox(
        "Select Historical Crash Event",
        [s['name'] for s in scenarios],
        index=4 # Default to COVID
    )
    
    scenario = next(s for s in scenarios if s['name'] == selected_scenario_name)
    st.markdown(f"> **Event Detail:** {scenario['description']}")
    ...
    for fund in funds_data:
        ...
        if res['data_available']:
            stress_results.append({
                'Fund': fund['name'][:30],
                'Return in Crash': res['portfolio_return'],
                'Max Drawdown': res['portfolio_max_drawdown'],
                'Days to Recover': res['days_to_recover'] or "Never/Pending"
            })
            
    if stress_results:
        stress_df = pd.DataFrame(stress_results)
        st.dataframe(style_financial_dataframe(stress_df), use_container_width=True, hide_index=True)
        
        # Add bench comparison for context
        res_bench = simulate_market_scenario(main_bench_series, main_bench_series, scenario)
        st.caption(f"Benchmark ({selected_benchmarks[0] if selected_benchmarks else 'Primary'}) during this period: Return: {res_bench['benchmark_return']:.2f}%, Max DD: {res_bench['benchmark_max_drawdown']:.2f}%")
    else:
