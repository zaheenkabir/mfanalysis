...
from ui_components import (
    ...
    render_capture_ratio_chart, animate_chart, render_verdict_card
)

from verdict_engine import generate_comparison_verdict_advanced, generate_single_verdict, MarkovAnalyzer

from analytics import (
    get_fund_metrics, calculate_rolling_returns, 
    calculate_correlation_matrix, simulate_market_scenario, get_predefined_scenarios,
    calculate_max_drawdown_series, calculate_rolling_returns_stats, filter_by_period
)
...
def render_comparison_dashboard(nav_data, selected_funds, selected_benchmarks=["NIFTY 50"]):
    ...
    from analytics import download_benchmark
    
    # Initialize default benchmark series for reference
    ...
    for fund in filtered_funds_data:
        ...
        # Calculate Welment Score for differentiation
        # Standardized 3Y rolling if possible for consistency
        category = fund['metadata'].get('scheme_category', '').lower()
        benchmark_ticker = "NIFTY 50"
        if 'small' in category: benchmark_ticker = "NIFTY SMALLCAP 50"
        elif 'mid' in category: benchmark_ticker = "NIFTY MIDCAP 50"
        
        bench_full = download_benchmark(benchmark_ticker)
        r_stats = calculate_rolling_returns_stats(fund['history'], bench_full, window_years=3)
        
        # Multi-period stability if possible (using filtered history as source)
        multi = {}
        for p in ["1Y", "3Y", "5Y"]:
            try:
                h_p = filter_by_period(fund['history'], p)
                if not h_p.empty:
                    # Use provided benchmark for metrics
                    multi[p] = get_fund_metrics(h_p, bench_full)
            except: pass
        
        # Calculate Markov Reliability
        try:
            m['markov_reliability'] = MarkovAnalyzer.calculate_reliability_score(fund['history'], bench_full)
        except:
            m['markov_reliability'] = 50.0

        f_verdict = generate_single_verdict(m, r_stats, category=fund['metadata'].get('scheme_category'), multi_metrics=multi)
        
        metrics_data.append({
            'Rank': 0, # Placeholder
            'Score': f_verdict['score'],
            'Fund': fund['name'],
    ...
    metrics_df = pd.DataFrame(metrics_data).sort_values(by='Score', ascending=False)
    ...
    fig_perf = animate_chart(fig_perf)
    ...
    fig_multi_rolling = animate_chart(fig_multi_rolling)
    ...
    st.caption("Our Welment Engine compares all selected funds across risk, return, and consistency pillars.")
    
    try:
        # Prepare metrics dict for comparison engine
        comparison_metrics = {}
        for fund in filtered_funds_data:
             # Calculate 3Y rolling stats for more robust comparison if possible
             
             # Get fund's specific benchmark for consistency
             category = fund['metadata'].get('scheme_category', '').lower()
             benchmark_ticker = "NIFTY 50"
             if 'small' in category: benchmark_ticker = "NIFTY SMALLCAP 50"
             elif 'mid' in category: benchmark_ticker = "NIFTY MIDCAP 50"
             ...
             # Use the new engine to get an accurate score for each fund
             fund_verdict = generate_single_verdict(fund['metrics'], r_stats, category=fund['metadata'].get('scheme_category'))
             comparison_metrics[fund['name']] = fund_verdict['score']
        
        comp_results = generate_comparison_verdict_advanced(comparison_metrics)
        
        winner_name = comp_results['winner']
        winner_fund_data = next(f for f in filtered_funds_data if f['name'] == winner_name)
        
        # Recalculate full verdict for winner (including multi-window stability)
        cat_winner = winner_fund_data['metadata'].get('scheme_category', '').lower()
        benchmark_ticker = "NIFTY 50"
        if 'small' in cat_winner: benchmark_ticker = "NIFTY SMALLCAP 50"
        elif 'mid' in cat_winner: benchmark_ticker = "NIFTY MIDCAP 50"
        
        bench_full = download_benchmark(benchmark_ticker)
        r_stats_winner = calculate_rolling_returns_stats(winner_fund_data['history'], bench_full, window_years=3)
        
        # Multi-window metrics for stability
        m_winner = {}
        for p in ["1Y", "3Y", "5Y"]:
            try:
                h_p = filter_by_period(winner_fund_data['history'], p)
                b_p = filter_by_period(bench_full, p)
                if not h_p.empty:
                    m_winner[p] = get_fund_metrics(h_p, b_p)
            except: pass
            
        winner_verdict = generate_single_verdict(winner_fund_data['metrics'], r_stats_winner, category=winner_fund_data['metadata'].get('scheme_category'), multi_metrics=m_winner)
        winner_verdict['label'] = f"The Recommended Winner: {winner_name}"
        winner_verdict['verdict_text'] = f"Leading the pack with a score of {winner_verdict['score']}/100. " + winner_verdict['verdict_text']
        
        render_verdict_card(winner_verdict)
    except Exception as e:
        st.error(f"Error generating comparison verdict: {str(e)}")
